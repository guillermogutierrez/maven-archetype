...
import static ${package}.menu.domain.MenuHelper.createMenus;
...
import ${package}.menu.api.v1.dto.response.SearchMenuResult;
...
public class QueryMenuControllerImplTest {
  ...
  public void listMenusAndPagination() {
    ...
    var response =
        this.testRestTemplate.getForEntity(
            getBaseURL(port)
                + String.format("${apiPath}?pageSize=%d&pageNumber=%d", pageSize, pageNumber),
  ...
  public void listMenusFilteredByRestaurantId() {
    ...
    var result =
        this.testRestTemplate.getForEntity(
            String.format("%s${apiPath}?restaurantId=%s", getBaseURL(port), restaurantId),
            SearchMenuResult.class);
    // Then
  ...
  public void getMenuById() {
    ...
    var response =
        this.testRestTemplate.getForEntity(
            String.format("%s${apiPath}/%s", getBaseURL(port), menu.getId()), MenuDTO.class);
  ...
  public void listMenusWithDefaultPagination() {
    ...
    // When
    var response =
        this.testRestTemplate.getForEntity(getBaseURL(port) + "${apiPath}", SearchMenuResult.class);
